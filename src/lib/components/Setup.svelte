    ...
    <form class="space-y-5 px-6 py-8 sm:px-10" onsubmit={(event) => { event.preventDefault(); void submit(); }}>
      ...
      <div class="grid gap-4 sm:grid-cols-2">
        <div>
          ...
          <input class="field-input tracking-[0.25em]" id="setup-pin" bind:value={pin} type="password" inputmode="numeric" pattern="\d{6}" maxlength="6" autocomplete="new-password" placeholder="••••••" required />
        </div>
        <div>
          <label class="field-label" for="confirm-pin">Confirm PIN</label>
          <input class="field-input tracking-[0.25em]" id="confirm-pin" bind:value={confirmPin} type="password" inputmode="numeric" pattern="\d{6}" maxlength="6" autocomplete="new-password" placeholder="••••••" required />
        </div>
      </div>
