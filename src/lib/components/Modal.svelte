...
<div class="fixed inset-0 z-50 grid items-end bg-black/75 p-0 backdrop-blur-[2px] sm:place-items-center sm:p-4" role="presentation" onclick={(event) => { if (event.target === event.currentTarget) onClose(); }}>
  <div class="max-h-[92dvh] w-full overflow-y-auto rounded-t-[24px] bg-apple-surface shadow-2xl sm:max-w-[520px] sm:rounded-[24px]" role="dialog" aria-modal="true" aria-labelledby="modal-title" tabindex="-1">
