  ...
  >
    <form class="space-y-4" onsubmit={(event) => { event.preventDefault(); void submitModal(); }}>
      {#if modal === 'staff'}
        ...
        <div class="grid gap-4 sm:grid-cols-2"><label><span class="field-label">Role</span><span class="relative block"><select class="field-input appearance-none pr-9" bind:value={formRole}><option value="teacher">Teacher</option><option value="teacher_admin">Teacher admin</option>{#if bootstrap.user.role === 'super_admin'}<option value="super_admin">Super admin</option>{/if}</select><ChevronDown class="pointer-events-none absolute top-3.5 right-3 text-apple-tertiary" size={16} /></span></label><label><span class="field-label">6-digit PIN</span><input class="field-input tracking-[0.2em]" bind:value={formPin} type="password" inputmode="numeric" pattern="\d{6}" maxlength="6" autocomplete="new-password" required /></label></div>
      ...
      {:else if modal === 'pin'}
        <label><span class="field-label">New 6-digit PIN</span><input class="field-input tracking-[0.2em]" bind:value={formPin} type="password" inputmode="numeric" pattern="\d{6}" maxlength="6" autocomplete="new-password" required /></label>
      {/if}
